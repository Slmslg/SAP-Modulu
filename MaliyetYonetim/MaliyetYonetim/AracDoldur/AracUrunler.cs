...
{
    class AracUrunler : Araclar
    {
        public AracUrunler(DataGridView dg)
        {
            dg.Columns.Clear();
            baglan.Open();
            cmd = new SqlCommand("select urun.UrunID,tur.TurID,urun.UrunAd,urun.BirimFiyat,urun.Aciklama,urun.Tur from urunler as urun inner join TUR as tur on tur.TurAd=urun.Tur", baglan);
